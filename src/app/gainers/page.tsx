...
import { useGainers } from '@/hooks/useQueries';
...
export default function GainersPage() {
    const { data: gainers, isLoading } = useGainers();
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-gray-500">Tokens with highest 24h price increase</p>
                    </div>
                </div>
            ...
            {/* Content */}
            {viewMode === 'grid' ? (
                <TokenGrid pairs={gainers} isLoading={isLoading} emptyMessage="No gainers found" />
            ) : (
                <div className="bg-[#1a1a1a] rounded-2xl border border-white/5 overflow-hidden">
                    <PairsTable pairs={gainers || []} isLoading={isLoading} />
                </div>
            )}
        </div>
    );
